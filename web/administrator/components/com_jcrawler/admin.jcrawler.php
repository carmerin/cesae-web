...
/**
 * @version 	$Id: admin.jcrawler.php 2012-01-28 11:22:00Z zanardi $
 * @package 	JCrawler
 * @copyright 	(C) 2011-2012 Patrick Winkler
...
if ( array_search( 'curl', get_loaded_extensions() ) ) {
	define ( 'CURL_LOADED', True );
} 
...
if ( $auth == false ) $app->redirect( 'index.php', JText::_('ALERTNOTAUTH') );
...
function modifyrobots ($sitemap_url,$page_root) {
	...
	if ($pos==0){	
		if (JFile::write( $page_root.'robots.txt',$robots."\n# BEGIN JCRAWLER-XML-SITEMAP-COMPONENT\nSitemap: ".$sitemap_url."\n# END JCRAWLER-XML-SITEMAP-COMPONENT" )!=false) {
			...
			$app->enqueueMessage("Your robots.txt is not writable!",'error');
		}
	} else {
		$app->enqueueMessage( "robots.txt contains already sitemap location" );
	}
}
...
/**
 * @return boolean	True if sh404SEF file is readable
 */
...
// This is the main function which gets the links. A cycle goes through each level.
function getlinks($url,$forbidden_types,$level,$exclude_names,$method,$maxcon,$timeout) {
	...
	for ($u=0;$u<$level;$u++){
		...
		//echo "tmparr: "; print_r ( $tmparr );echo "<br />";
 ...
 * @global object $linkarraykey
 * @param buffer a crawl result (an html page)
 * @param key the url which was crawled
 * @return array a list of urls contained in this html page
 */
...
{
	 if ( array_search( $name, get_loaded_extensions() ) === false ) {
		return false;				
	 } else {
		return true; 
	 }
