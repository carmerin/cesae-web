...
//quit();
	...
	if ($count == 1 && $_SERVER["SERVER_NAME"]!="blog.cesae.es") {
...
//MGG BLOG 11-01-2014
	...
	// we test it first to see if it is a category.  If the id and alias match a category
	// then we assume it is a category.  If they don't we assume it is an article
	if ($count == 2 && $segments[0]=="blog:portada") {
		
			header("HTTP/1.1 301 Moved Permanently");
			header("Location: http://www.cesae.es/blog/". $segments[1] ."/");
			exit();
	...
	if ($_SERVER["SERVER_NAME"]=="blog.cesae.es")
	{
		...
		if ($_SERVER["REQUEST_URI"]=="/index.php/tag/restauracion/")
		{
			header("HTTP/1.1 301 Moved Permanently");
			header("Location: http://www.cesae.es/programadireccionygestionrestauracion");
			exit();
		}


		if ($_SERVER["REQUEST_URI"]=="/index.php/tag/enoturismo/")
		{
			header("HTTP/1.1 301 Moved Permanently");
			header("Location: http://www.cesae.es/blog/las-claves-del-vino-y-turismo-enoturismo");
			exit();
		    
		}
    
		if ($_SERVER["REQUEST_URI"]=="/index.php/tag/late-check-out/")
		{
			header("HTTP/1.1 301 Moved Permanently");
			header("Location: http://www.cesae.es/blog/descubre-la-relacion-entre-turismo-rural-y-revenue-management");
			exit();
		}
		if ($_SERVER["REQUEST_URI"]=="/index.php/tag/valor-anadido-turismo/")
		{
			header("HTTP/1.1 301 Moved Permanently");
			header("Location: http://www.cesae.es/desarrollocorporativo");
			exit();
		}
		if ($_SERVER["REQUEST_URI"]=="/index.php/tag/hoteles-rentables/")
		{
			header("HTTP/1.1 301 Moved Permanently");
			header("Location: http://www.cesae.es/revenueavanzadoupsellingycrossselling-programa");
			exit();
		}
